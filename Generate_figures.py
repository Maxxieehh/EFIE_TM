...
output = tm.EFIE_TE(boundary_points,wavelength,angle)
Jz = output[0]
Z = output[2]
end = time.time()
time_main = end-start
...
for i in np.arange(N):
    for j in np.arange(N):
        ...
        Ein[i,j] = tm.Efield_in(r,wavelength,angle)
        Esc[i,j] = tm.Escatter(Jz,r,boundary_points,wavelength)
